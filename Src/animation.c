...
// 日历动画全局变量
// glowIntensity: 发光强度(0-1)，控制边框外发光的亮度
// hoverScale: 悬停缩放比例，1.0为正常大小（目前未使用）
// selectPulse: 选择脉冲动画进度(0-1)，点击日期时的动画效果
// isHovered: 鼠标是否悬停在日历上
// isSelecting: 是否正在播放选择动画
// selectAnimStep: 选择动画当前帧数
// shimmerPhase: 闪光效果的相位角度(0-2π)，控制闪光线的位置
// breathPhase: 呼吸灯效果的相位角度(0-2π)，控制发光的脉动
...
// ============================================
// DrawCalendarGlow - 绘制日历发光边框效果
// ============================================
// 功能：在日历控件周围绘制多层发光效果，产生柔和的光晕
// 参数：
//   hdc  - 设备上下文句柄
//   prc  - 日历控件的矩形区域
//   anim - 日历动画状态结构体指针
//
// 实现原理：
//   1. 计算总发光强度 = 悬停发光 + 选择脉冲发光
//   2. 叠加呼吸灯效果（sinf函数产生周期性变化）
//   3. 绘制三层发光：
//      - 核心发光层：最亮，贴近边框
//      - 主发光层：多层渐变，使用指数衰减透明度
//      - 柔光层：最外层，非常淡
//   4. 每层使用RoundRect绘制圆角矩形边框
//
// 问题点：
//   - 发光层数过多(3+totalGlow*4)，可能导致性能问题
//   - 颜色混合计算复杂，可以简化
//   - breathingEffect的振幅(0.15f)可能太小，效果不明显
// ============================================
...
// ============================================
// DrawCalendarParticles - 绘制日历装饰粒子效果
// ============================================
// 功能：在日历周围绘制向外扩散的小圆点粒子
// 参数：
//   hdc  - 设备上下文句柄
//   prc  - 日历控件的矩形区域
//   anim - 日历动画状态结构体指针
//
// 实现原理：
//   1. 仅在选择动画早期(selectPulse <= 0.6)显示粒子
//   2. 12个粒子均匀分布在圆周上
//   3. 粒子半径随动画进度增大（20 -> 55像素）
//   4. 粒子透明度快速衰减（二次函数）
//   5. 粒子大小从3像素递减到1像素
//
// 问题点：
...
//
// 实现原理：
//   1. 根据shimmerPhase计算闪光线的起始位置
//   2. 绘制一条斜向的白色半透明线条
//   3. 线条位置随shimmerPhase周期性变化
//
// 问题点：
//   - 只绘制了1条线（注释说2条但实际只有1条）
//   - 闪光线可能超出日历边界，没有裁剪
//   - 线条宽度固定为2像素，可能太粗或太细
//   - 效果不够明显，shimmerIntensity * 0.4f太低
// ============================================
